...
public class BasicGraph<N> implements Graph<N> {
    ...
    public BasicGraph(Set<N> nodes, Set<Edge<N>> edges) {
        ...
        this.backing = Collections.unmodifiableMap(nodes.stream().collect(Collectors.toMap(Function.identity(), node -> Collections.unmodifiableSet(edges.stream().filter(edge -> edge.a().equals(node) || edge.b().equals(node)).collect(Collectors.toSet())))));
    }
